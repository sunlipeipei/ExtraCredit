...
export default function Homepage() {
    ...
    async function deletePokemon(name, id) {
        const confirmDeletion =  window.confirm(`Are you sure you want to delete Pokemon - ${name}?`)
        if(!confirmDeletion){
            return;
        }
        try{const response = await axios.delete(`/api/pokemon/${id}`)
    ...
    for(let i = 0; i < pokemonState.length; i++) {
        ...
        const newPokeComponent = (<div>
            ...
            - Level: {pokemon.level}
            <button onClick={() => deletePokemon(pokemon.name, pokemon.id)}>Delete</button>
